...
if __name__=='__main__':
    ...
    netD=model.NLayerDiscriminator(use_sigmoid=True)
    ...
    for epoch in range(100):
        ...
        for idx,data in enumerate(trainloader):
            ...
            valid_T = torch.ones(data.shape[0], 1).cuda()
            zeros_T = torch.zeros(data.shape[0], 1).cuda()
            ...
            ssiml=SSIM(mask, pred_mask)
            image_loss = msel+0.1*ssiml
            adversarial_loss = criterion_GAN(netD(f_p).mean(),valid_T)
            ...
            logits_real = netD(r_p).mean()
            logits_fake = netD(f_p).mean()
            loss_D_fake = criterion_GAN(logits_real, zeros_T)
            loss_D_real = criterion_GAN(logits_real, valid_T)
        ...
        for idx,data in enumerate(valloader):
            ...
            cv2.imwrite(os.path.join(tmp_path,str(epoch)+'Train',name),pred)
